...
| S1 | 本体推送：CLI push 首版本体 | #6（占位，决议后回填） | 60 |
| S2 | 批量接入：服务账号经接入端点导入存量，批次审计一条 | ADR-0005 | 70/20 |
| S3 | 读授权两态：全类型授权 vs 谓词收窄；静默收窄、零授权=零行 | ADR-0004；#10 编码面（占位） | 50/30 |
| S4 | 录用正反路径：动态默认→ValidationFailed→422→回滚无审计→修正成功→审计一条 | ADR-0003；#10 编码面（占位） | 20/30 |
...
| S9 | 查询包：嵌套过滤+一跳链接过滤+keyset+count+include 2 跳；queryFn 花名册 | ADR-0002；#10（占位） | 30/40 |
| S10 | 演化小步：加属性再 push，变更分类学落点 | #6（占位，决议后回填） | 60 |
| S11 | 审计与安全日志查询：管理面只读端点、过滤 | ADR-0003、0004；#10（占位） | 30/50 |
...
| #12 TS DSL 外形（在飞） | 示例本体即反应物（S1/S4/S7 全体） | 占位 |
| #6 本体定义与演化（在飞） | S1、S10 | 占位 |
| #10 API 与逻辑接口（在飞） | S3、S4、S9、S11 编码面 | 占位 |
